  ...
  for (int ph = 0; ph < width; ph++) {
    ...
    if (threadIdx.x + ph * BLOCK_DIM < K && col < N) {
      SB[threadIdx.x][threadIdx.y] =
          dB[(threadIdx.x + ph * BLOCK_DIM) * N + col];
    } else {
...
void hostMatrix(float *hostA, float *hostB, float *hostC, int M, int K, int N) {
  ...
  cudaMemcpy(dC, hostC, M * N * sizeof(float), cudaMemcpyHostToDevice);
...
int main() {
  ...
  CInitialData cinitdata;
  cinitdata.initialDataABCByFile(hostA, hostB, hostC, M, N);
  cinitdata.initialDataABCByFile(hostA, hostB, serialC, M, N);
  ...
  cprintmatrix.printMatrixCinFile(hostC, M, N);
  cprintmatrix.printMatrixCinFile(serialC, M, N);
