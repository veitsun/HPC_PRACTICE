...
// #pragma G++ optimize("inline", "fast-math", "unroll-loops", "no-stack-protector")
// #pragma G++ target("avx,avx2,fma")
// #include "common.h"
#include "common.h"
#include "utils.h"
...
#pragma omp section
        {
            ...
            for (int i = 0; i < short_row_3; i++)
            {
                order_rid[row_long + row_block + common_13 * 2 + i] = short_rid_3[common_13 + i];
            }
        }
...
#pragma omp section
        {
            ...
            for (int i = 0; i < short_row_4; i++)
            {
                order_rid[row_long + row_block + common_13 * 2 + short_row_3 + i] = short_rid_4[i];
            }
        }
...
#pragma omp section
        {
            ...
            {
                order_rid[row_long + row_block + common_13 * 2 + short_row_3 + short_row_4 + i] = short_rid_2[i];
            }
        }
...
#pragma omp section
        {
            ...
            for (int i = 0; i < short_row_1; i++)
            {
                order_rid[row_long + row_block + common_13 * 2 + short_row_3 + short_row_4 + short_row_2 + i] = short_rid_1[i];
            }
        }
...
#pragma omp section
        {
            ...
            {
                order_rid[row_long + row_block + common_13 * 2 + short_row_3 + short_row_4 + short_row_2 + short_row_1 + i] = zero_rid[i];
            }
        }
    }
...
#pragma unroll
    ...
    {   
        ...
        blockPtr[i + 1] = ((blockPtr[i + 1] + MMA_M * MMA_K * 4 - 1) / (MMA_M * MMA_K * 4)) * (MMA_M * MMA_K * 4);
    }
