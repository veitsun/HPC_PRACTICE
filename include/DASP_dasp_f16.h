...
// // "no-stack-protector") #pragma G++ target("avx,avx2,fma") #include
// "common.h" #include "DASP_common.h" #include "DASP_utils.h" #include
// <cuda_runtime.h>
...
//           uint32_t *dshort_val, int *dshort_cid, int short_row_1, int
//           common_13, int short_row_34, int short_row_2, int offset_reg, int
//           offset_short1, int offset_short13, int offset_short34, int
//           offset_short22, MAT_PTR_TYPE fill0_nnz_short13, MAT_PTR_TYPE
//           fill0_nnz_short34, MAT_PTR_TYPE fill0_nnz_short22) {
//   int bid = blockIdx.x;
//   int tid = threadIdx.x;
//   int laneid = 31 & tid;
...
//       int offset_cid = (global_warpid * loopNum_long + i) * MMA_M * MMA_K *
//       4; int offset_val = offset_cid >> 1;
...
//       blocklen = load_int_from_global(dblockA_ptr + block_idx + 1) -
//       offset_A;
...
//     int *cur_cid =
//         dshort_cid + fill0_nnz_short13 + fill0_nnz_short34 +
//         fill0_nnz_short22;
//     if (tid1 >= short_row_1) {
//       return;
//     }
//     MAT_VAL_TYPE *valA = reinterpret_cast<MAT_VAL_TYPE *>(&cur_val[tid1 >>
//     1]); int x_idx = load_int_from_global(cur_cid + tid1); MAT_VAL_TYPE
//     temp_y =
...
//       // valX_half[curA_cid[idx + 2]], fragB[3] = valX_half[curA_cid[idx +
//       3]]; mma_m8n8k4_fp16_v2(fragC, fragA, fragB); res =
//       __shfl_sync(0xffffffff, fragC[target_idx], new_id);

//       offset_y += WARP_SIZE;
//       if (offset_y < common_13 * 2)
//         store_half_to_global(valY_half + row_long + row_block + offset_y,
//         res);
//       // valY_half[row_long + row_block + offset_y] = res;
//     }
...
//             valY_half + row_long + row_block + common_13 * 2 + offset_y,
//             res);
//     }
//   } else {
...
//     {
// // memcpy(order_rid + row_long + row_block + common_13 * 2 + short_row_3 +
// // short_row_4 + short_row_2 + short_row_1, zero_rid, sizeof(int) *
// row_zero); #pragma omp parallel for
...
//   cudaMalloc((void **)&dX_val, sizeof(MAT_VAL_TYPE) * (((colA + 1) / 2) *
//   2)); cudaMalloc((void **)&dY_val, sizeof(MAT_VAL_TYPE) * (((rowA + 1) / 2)
//   * 2)); cudaMemcpy(dX_val, X_val, sizeof(MAT_VAL_TYPE) * (((colA + 1) / 2) *
//   2),
